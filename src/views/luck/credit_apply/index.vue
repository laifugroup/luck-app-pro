<template>
  <div class="container">
    <Breadcrumb :items="['menu.luck.creditApply', 'menu.luck.creditApplyList']" />
    <a-card class="general-card" :title="$t('menu.luck.creditApplyList')">
      <a-row>
        <a-col :flex="1">
          <a-form :model="formModel" :label-col-props="{ span: 6 }" :wrapper-col-props="{ span: 18 }" label-align="left">
            <a-row :gutter="16">
              ...
              <a-col :span="8">
                <a-form-item field="userId" :label="$t('creditApply.form.userId')">
              ...
              <a-col :span="8">
                <a-form-item field="type" :label="$t('creditApply.form.type')">
                  <a-select v-model="formModel.type" :options="contentTypeOptions"
                    :placeholder="$t('creditApply.form.type.placeholder')" />
                </a-form-item>
              </a-col>
              ...
              <a-col :span="8">
                <a-form-item field="status" :label="$t('creditApply.form.status')">
                  <a-select v-model="formModel.status" :options="contentStatusOptions"
                    :placeholder="$t('creditApply.form.status.placeholder')" />
                </a-form-item>
              </a-col>
...
const columns = computed<TableColumnData[]>(() => [
  ...
  {
    title: t("creditApply.form.userId"),
    dataIndex: "userId",
    ellipsis: true,
    tooltip: true,
  },
  ...
  {
    title: t("creditApply.form.credit"),
    dataIndex: "credit",
    render(data) {
      let credit = data.record.credit;
      if(credit === undefined) {
        credit = '-';
      } else {
        credit = credit.toFixed(2)+'U';
      }
      return credit;
    },
  },
  ...
  {
    title: t("creditApply.form.status"),
    dataIndex: "status",
    render(data) {
...
const handleDetailView = (data: Resp) => {
  if (data.id !== undefined && data.id !== "") {
    router.push({ path: 'creditApplyView', query: { id: data.id } })
  }
}

const handleFormView = (data: Resp | undefined) => {
  router.push({ path: 'creditApplyForm', query: { id: data?.id } })
}
</script>
