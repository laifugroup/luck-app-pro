<template>
  <div class="container">
    <Breadcrumb :items="['menu.luck.user', 'menu.luck.creditLogList']" />
    <a-card class="general-card" :title="$t('menu.luck.creditLogList')">
      <a-row>
        <a-col :flex="1">
          <a-form :model="formModel" :label-col-props="{ span: 6 }" :wrapper-col-props="{ span: 18 }" label-align="left">
            <a-row :gutter="16">
              ...
              <a-col :span="8">
                <a-form-item field="userId" :label="$t('creditLog.form.userId')">
              ...
              <a-col :span="8">
                <a-form-item field="type" :label="$t('creditLog.form.type')">
                  <a-select v-model="formModel.type" :options="contentTypeOptions"
                    :placeholder="$t('creditLog.form.type.placeholder')" />
                </a-form-item>
              </a-col>
      ...
      <a-row style="margin-bottom: 16px">
        <a-col :span="12">
          <a-space>
            <a-button type="primary" @click="handleFormView(undefined)">
              <template #icon>
                <icon-plus />
              </template>
...
const columns = computed<TableColumnData[]>(() => [
  ...
  {
    title: t("creditLog.form.userId"),
    dataIndex: "userId",
    ellipsis: true,
    tooltip: true,
  },
  {
    title: t("creditLog.form.creditBefore"),
    dataIndex: "creditBefore",
    render(data) {
      return data.record.creditBefore.toFixed(2) + 'U'
    },
  },

  {
    title: t("creditLog.form.credit"),
    dataIndex: "credit",
    render(data) {
  ...
  {
    title: t("creditLog.form.creditAfter"),
    dataIndex: "creditAfter",
    render(data) {
      return data.record.creditAfter.toFixed(2) + 'U'
    },
  },


  {
    title: t("creditLog.form.type"),
    dataIndex: "type",
    render(data) {
      return contentTypeOptions.value.find(item => item.value === data.record.type)?.label
    },
  },
  // {
  //   title: t("creditLog.form.remark"),
  //   dataIndex: "remark",
  // },
...
const handleDetailView = (data: Resp) => {
  if (data.id !== undefined && data.id !== "") {
    router.push({ path: 'creditLogView', query: { id: data.id } })
  }
}

const handleFormView = (data: Resp | undefined) => {
  router.push({ path: 'creditLogForm', query: { id: data?.id } })
}
</script>
