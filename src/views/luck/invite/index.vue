<template>
  <div class="container">
    <Breadcrumb :items="['menu.luck.invite', 'menu.luck.inviteList']" />
    <a-card class="general-card" :title="$t('menu.luck.inviteList')">
      <a-row>
        <a-col :flex="1">
          <a-form :model="formModel" :label-col-props="{ span: 6 }" :wrapper-col-props="{ span: 18 }" label-align="left">
            <a-row :gutter="16">
              ...
              <a-col :span="8">
                <a-form-item field="userId" :label="$t('invite.form.userId')">
                  <a-input v-model="formModel.userId" :placeholder="$t('invite.form.userId.placeholder')" />
                </a-form-item>
              </a-col>
              <a-col :span="8">
                <a-form-item field="status" :label="$t('invite.form.status')">
                  <a-select v-model="formModel.status" :options="contentStatusOptions"
                    :placeholder="$t('invite.form.status.placeholder')" />
                </a-form-item>
              </a-col>
...
const columns = computed<TableColumnData[]>(() => [
  ...
  {
    title: t("invite.form.userId"),
    dataIndex: "userId",
    ellipsis: true,
    tooltip: true,
  },
  {
    title: t("invite.form.groupId"),
    dataIndex: "groupId",
  },
  {
    title: t("invite.form.url"),
    dataIndex: "url",
    ellipsis: true,
    tooltip: true,
  },
...
const handleDetailView = (data: Resp) => {
  if (data.id !== undefined && data.id !== "") {
    router.push({ path: 'inviteView', query: { id: data.id } })
  }
}

const handleFormView = (data: Resp | undefined) => {
  router.push({ path: 'inviteForm', query: { id: data?.id } })
}
</script>
