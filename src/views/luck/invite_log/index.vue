<template>
  <div class="container">
    <Breadcrumb :items="['menu.luck.inviteLog', 'menu.luck.inviteLogList']" />
    <a-card class="general-card" :title="$t('menu.luck.inviteLogList')">
      <a-row>
        <a-col :flex="1">
          <a-form :model="formModel" :label-col-props="{ span: 6 }" :wrapper-col-props="{ span: 18 }" label-align="left">
            <a-row :gutter="16">
              ...
              <a-col :span="8">
                <a-form-item field="userId" :label="$t('inviteLog.form.userId')">
                  <a-input v-model="formModel.userId" :placeholder="$t('inviteLog.form.userId.placeholder')" />
                </a-form-item>
              </a-col>
              <a-col :span="8">
                <a-form-item field="inviteeUserId" :label="$t('inviteLog.form.inviteeUserId')">
                  <a-input v-model="formModel.inviteeUserId" 
                  :placeholder="$t('inviteLog.form.inviteeUserId.placeholder')"
                    />
                </a-form-item>
              </a-col>
            </a-row>
...
const columns = computed<TableColumnData[]>(() => [
  ...
  {
    title: t("inviteLog.form.userId"),
    dataIndex: "userId",
    ellipsis: true,
    tooltip: true,
  },
  {
    title: t("inviteLog.form.groupId"),
    dataIndex: "groupId",
  },

  {
    title: t("inviteLog.form.inviteeUserId"),
...
const handleDetailView = (data: Resp) => {
  if (data.id !== undefined && data.id !== "") {
    router.push({ path: 'inviteLogView', query: { id: data.id } })
  }
}

const handleFormView = (data: Resp | undefined) => {
  router.push({ path: 'inviteLogForm', query: { id: data?.id } })
}
</script>
