<template>
  <div class="container">
    <Breadcrumb :items="['menu.luck.wallet', 'menu.luck.walletList']" />
    <a-card class="general-card" :title="$t('menu.luck.walletList')">
      <a-row>
        <a-col :flex="1">
            ...
            <a-row :gutter="16">
              ...
              <a-col :span="8">
                <a-form-item field="userId" :label="$t('wallet.form.userId')">
                  <a-input v-model="formModel.userId" :placeholder="$t('wallet.form.userId.placeholder')" />
                </a-form-item>
              </a-col>
              <a-col :span="8">
                <a-form-item field="status" :label="$t('wallet.form.status')">
                  <a-select v-model="formModel.status" :options="contentStatusOptions"
                    :placeholder="$t('wallet.form.status.placeholder')" />
                </a-form-item>
              </a-col>
...
const columns = computed<TableColumnData[]>(() => [
  ...
  {
    title: t("wallet.form.userId"),
    dataIndex: "userId",
  },

  {
    title: t("wallet.form.credit"),
    dataIndex: "credit",
    render(data) {
...
const handleDetailView = (data: Resp) => {
  if (data.id !== undefined && data.id !== "") {
    router.push({ path: 'walletView', query: { id: data.id } })
  }
}

const handleFormView = (data: Resp | undefined) => {
  router.push({ path: 'walletForm', query: { id: data?.id } })
}
</script>
